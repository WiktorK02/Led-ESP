...
void loop() {
    ...
    static int filteredCounter = 1;  // Initialize to 1 to turn the LED strip on initially
    ...
    if (filteredReversedValue != prevFilteredValue) {
        ...
        if (filteredReversedValue == 255) {
            ...
            nextPattern();
            filteredCounter = 1;  // Reset the counter when the value is 255
        }
        else if (filteredReversedValue == 254){
            if (filteredCounter % 2 == 0) {
                fill_solid(leds, NUM_LEDS, CRGB::Black);  // Turn off the LED strip
